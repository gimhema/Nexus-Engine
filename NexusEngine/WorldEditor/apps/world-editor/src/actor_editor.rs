...
//! - 표시 칸 아래에 **시트 재생 미리보기**가 있다 — 시트 뷰어와 같은 규칙으로 읽고, 명시한 색을 곱해 보여 준다.
//!   편집기 안에서는 곧바로 바뀐다 (egui 텍스처라 게임 렌더러의 텍스처와 무관).
...
pub(crate) struct ActorEditor {
    ...
    scripts: Vec<String>,
    sheets: Vec<String>,
    /// 고른 시트의 재생 미리보기 — 시트나 색을 바꾸면 곧바로 보인다.
    preview: crate::sheet_viewer::SheetPreview,
...
impl ActorEditor {
    ...
    fn form(&mut self, ui: &mut egui::Ui) {
        ...
        // 명시한 색만 넘긴다 — 흰색이면 "정하지 않음" (게임의 색 규칙과 같다, sprites::tint_of).
        let tint = (f.tint != (1.0, 1.0, 1.0)).then_some([f.tint.0, f.tint.1, f.tint.2]);
        self.preview.show(ui, &f.sheet, tint);
        ui.weak("⚠ 색은 곱하기라 어둡게·탁하게만 됩니다. 미리보기는 곧바로 바뀌지만, 게임 화면의 그림은 다시 시작해야 바뀝니다.");
