...
pub enum RenderCommand {
    ...
    /// # 텍스처
    ///
    /// `texture` / `uv` 로 **지면에 깔리는 그림**(타일 아트·데칼)을 그린다.
    /// 단색이면 [`TextureId::WHITE`] + [`UvRect::FULL`] 을 넘긴다 — 그때 `color` 가
    /// 그대로 나오고, 셰이더는 텍스처 유무로 갈리지 않는다.
    ///
    /// 그림을 넣을 때 `color` 는 **곱해지는 색**이다 (`[1.0; 4]` = 원래 색).
    ///
    /// 스프라이트([`DrawSprite`](Self::DrawSprite))와 달리 알파는 **컷아웃이 아니라
    /// 블렌딩**이다 — 지면 쿼드는 반투명 편집 표시(그리드·박스 선택)와 같은
    /// 파이프라인을 쓴다. 타일 아트는 보통 불투명하므로 차이가 없지만, 알파가 있는
    /// 그림을 지면에 깔면 **겹치는 순서가 깊이 버퍼로만 정해진다**는 점을 기억할 것.
    DrawRect {
        center: Vec2,
        size: Vec2,
        rotation: f32,
        z: f32,
        depth_bias: f32,
        /// 텍스처 색에 곱해지는 sRGB 색. 단색 쿼드에서는 이것이 곧 쿼드 색이다.
        color: [f32; 4],
        uv: UvRect,
        texture: TextureId,
    },
