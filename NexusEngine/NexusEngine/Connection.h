...
public:
    ...
    // 읽기 커서가 버퍼 절반 이상 이동했을 때 앞으로 당김 (realloc 없음)
    void Compact();
...
//
// TCP 클라이언트 연결 하나를 표현.
// NetworkManager 와 패킷 핸들러 콜백 사이에서 shared_ptr 로 공유.
//
// [Windows IOCP 패턴]
//   CreateIoCompletionPort(socket, iocpHandle, (ULONG_PTR)session.get(), 0)
//   → GetQueuedCompletionStatus 의 completionKey = Session*
//
// [Linux epoll 패턴]
//   epoll_event.data.ptr = session->GetRecvOv()
//   → OverlappedEx::sessionId 로 Session 역참조
// ─────────────────────────────────────────────────────────────────────────────
class Session : public std::enable_shared_from_this<Session>
{
public:
    // ioHandle: Windows = IOCP 핸들, Linux = epoll fd
    Session(NxSocket socket, uint64_t sessionId, NxHandle ioHandle);
    ~Session();
    ...
    // Windows: WSARecv 예약 / Linux: epoll EPOLLIN 등록
    void PostRecv();

    // Windows: WSASend / Linux: send() 루프 (EAGAIN 시 EPOLLOUT 등록)
    void FlushSendQueue();

private:
    NxSocket                   m_socket{ NX_INVALID_SOCKET };
    uint64_t                   m_sessionId{ 0 };
    NxHandle                   m_ioHandle{ NX_INVALID_HANDLE };  // IOCP 핸들 또는 epoll fd
    ...
    std::vector<uint8_t>              m_sendBuffer;  // Windows: WSASend 중인 패킷 보관
};
